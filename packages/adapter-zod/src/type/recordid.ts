 ...
 */
export const recordIdValueType: ZodCustom<RecordIdValue> = z.union([
	z.string(),
	z.number(),
	z.bigint(),
	z.unknown().array(),
...
>(
	...
	z
		.instanceof<typeof RecordId<TTable, TValue>>(RecordId)
		.superRefine((id, ctx) => {
			...
			if (options.recordId) {
				...
				const success: boolean = options.recordId.equals(id);
				if (!success)
					ctx.addIssue({
