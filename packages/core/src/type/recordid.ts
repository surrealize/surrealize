...
import { type Encodeable, Transformer } from "../query/transform/transformer";
import { Surrealize } from "../surrealize";
import type { SchemaLike } from "../utils/schema";
import { Table, type TableLike } from "./table";
...
> implements Encodeable<SurrealRecordId>
{
	...
	async resolve<TOutput = unknown>(
		...
		const surrealize = options.connection ?? Surrealize.default;
		if (!surrealize)
			throw new Error(
				"No connection provided. Please provide a connection or set a default connection.",
			);

		return surrealize.resolve(this, options.schema);
	}
